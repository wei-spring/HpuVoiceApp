...
public class XListView extends ListView implements OnScrollListener {
	...
	protected me.maxwin.view.IXListViewLoadMore mLoadMore;
	protected IXListViewRefreshListener mOnRefresh;

	// -- header view
	protected me.maxwin.view.XListViewHeader mHeaderView;
	...
	// -- footer view
	protected me.maxwin.view.XListViewFooter mFooterView;
	...
	protected void initWithContext(Context context) {
		...
		// init header view
		mHeaderView = new me.maxwin.view.XListViewHeader(context);
		...
		// init footer view
		mFooterView = new me.maxwin.view.XListViewFooter(context);
	 ...
	 * @param enable
	 */
	public void setPullLoadEnable(me.maxwin.view.IXListViewLoadMore loadMoreListener) {
		mEnablePullLoad = true;
		this.mLoadMore = loadMoreListener;
		mPullLoading = false;
		mFooterView.show();
		mFooterView.setState(me.maxwin.view.XListViewFooter.STATE_NORMAL);
	...
	protected void invokeOnScrolling() {
		if (mScrollListener instanceof me.maxwin.view.IXScrollListener) {
			me.maxwin.view.IXScrollListener l = (IXScrollListener) mScrollListener;
			l.onXScrolling(this);
		}
	}
	...
	protected void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 鏈浜庡埛鏂扮姸鎬侊紝鏇存柊绠ご
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				...
				mHeaderView.setState(me.maxwin.view.XListViewHeader.STATE_READY);
			} else {
				System.out.println("mPullRefreshing");
				mHeaderView.setState(me.maxwin.view.XListViewHeader.STATE_NORMAL);
			}
		}
	...
	protected void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(me.maxwin.view.XListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(me.maxwin.view.XListViewFooter.STATE_NORMAL);
			}
		}
	...
	protected void startLoadMore() {
		if (mEnablePullLoad
				...
				&& !mPullLoading) {
			mPullLoading = true;
			mFooterView.setState(me.maxwin.view.XListViewFooter.STATE_LOADING);
			if (mLoadMore != null) {
	...
	protected void startOnRefresh() {
		if (mEnablePullRefresh
			...
			mHeaderView.setState(me.maxwin.view.XListViewHeader.STATE_REFRESHING);
			if (mOnRefresh != null) {
